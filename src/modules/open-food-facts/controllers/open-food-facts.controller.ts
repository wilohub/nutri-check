...
export class OpenFoodFactsController {
  ...
  @ApiResponse({
    ...
    type: ExternalProductResponseDto,
  })
  @ApiResponse({ status: 404, description: 'El producto no existe en la base de datos externa.' })
  @ApiResponse({
    status: 502,
    description: 'Fallo de pasarela (error al conectar con Open Food Facts).',
  })
  async getExternalProduct(@Param('barcode') barcode: string): Promise<ExternalProductResponseDto> {
